...
var Frame = (function (_super) {
    ...
    Frame.prototype._navigateCore = function (backstackEntry) {
        ...
        if (!this._currentEntry) {
            ...
            if (trace_1.isEnabled()) {
                trace_1.write(this + ".pushViewControllerAnimated(" + viewController + ", " + animated + "); depth = " + navDepth, trace_1.categories.Navigation);
            }
            return;
        }
        if (clearHistory) {
            ...
            if (trace_1.isEnabled()) {
                trace_1.write(this + ".setViewControllersAnimated([" + viewController + "], " + animated + "); depth = " + navDepth, trace_1.categories.Navigation);
            }
            return;
        }
        if (!Frame._isEntryBackstackVisible(this._currentEntry) || isReplace) {
            ...
            this._ios.controller.setViewControllersAnimated(newControllers, animated);
            if (trace_1.isEnabled()) {
                trace_1.write(this + ".setViewControllersAnimated([originalControllers - lastController + " + viewController + "], " + animated + "); depth = " + navDepth, trace_1.categories.Navigation);
            }
            return;
        }
        this._ios.controller.pushViewControllerAnimated(viewController, animated);
    ...
    Frame.prototype._goBackCore = function (backstackEntry) {
        ...
        if (trace_1.isEnabled()) {
            trace_1.write(this + ".popToViewControllerAnimated(" + controller + ", " + animated + "); depth = " + navDepth, trace_1.categories.Navigation);
        }
    ...
    Frame.prototype._updateActionBar = function (page, disableNavBarAnimation) {
        ...
        var needsPageLayout = this._ios.showNavigationBar !== newValue;
...
var UINavigationControllerAnimatedDelegate = (function (_super) {
    ...
    UINavigationControllerAnimatedDelegate.prototype.navigationControllerAnimationControllerForOperationFromViewControllerToViewController = function (navigationController, operation, fromVC, toVC) {
        ...
        if (trace_1.isEnabled()) {
            trace_1.write("UINavigationControllerImpl.navigationControllerAnimationControllerForOperationFromViewControllerToViewController(" + operation + ", " + fromVC + ", " + toVC + "), transition: " + JSON.stringify(navigationTransition), trace_1.categories.NativeLifecycle);
        }
...
var UINavigationControllerImpl = (function (_super) {
    ...
    UINavigationControllerImpl.prototype.pushViewControllerAnimated = function (viewController, animated) {
        ...
        var navigationTransition = viewController[TRANSITION];
        if (trace_1.isEnabled()) {
            trace_1.write("UINavigationControllerImpl.pushViewControllerAnimated(" + viewController + ", " + animated + "); transition: " + JSON.stringify(navigationTransition), trace_1.categories.NativeLifecycle);
        }
        var nativeTransition = _getNativeTransition(navigationTransition, true);
    ...
    UINavigationControllerImpl.prototype.setViewControllersAnimated = function (viewControllers, animated) {
        ...
        var navigationTransition = viewController[TRANSITION];
        if (trace_1.isEnabled()) {
            trace_1.write("UINavigationControllerImpl.setViewControllersAnimated(" + viewControllers + ", " + animated + "); transition: " + JSON.stringify(navigationTransition), trace_1.categories.NativeLifecycle);
        }
        var nativeTransition = _getNativeTransition(navigationTransition, true);
        if (!animated || !navigationTransition || !nativeTransition) {
    ...
    UINavigationControllerImpl.prototype.popViewControllerAnimated = function (animated) {
        ...
        if (trace_1.isEnabled()) {
            trace_1.write("UINavigationControllerImpl.popViewControllerAnimated(" + animated + "); transition: " + JSON.stringify(navigationTransition), trace_1.categories.NativeLifecycle);
        }
    ...
    UINavigationControllerImpl.prototype.popToViewControllerAnimated = function (viewController, animated) {
        ...
        var navigationTransition = lastViewController[TRANSITION];
        if (trace_1.isEnabled()) {
            trace_1.write("UINavigationControllerImpl.popToViewControllerAnimated(" + viewController + ", " + animated + "); transition: " + JSON.stringify(navigationTransition), trace_1.categories.NativeLifecycle);
        }
        if (navigationTransition && navigationTransition.name === NON_ANIMATED_TRANSITION) {
    ...
    UINavigationControllerImpl.prototype.traitCollectionDidChange = function (previousTraitCollection) {
        ...
        if (majorVersion >= 13) {
            ...
            if (owner &&
                ...
                owner.notify({ eventName: view_helper_1.ios.traitCollectionColorAppearanceChangedEvent, object: owner });
            }
        }
    };
    __decorate([
...
function _getNativeCurve(transition) {
    if (transition.curve) {
        switch (transition.curve) {
            case "easeIn":
                if (trace_1.isEnabled()) {
                    trace_1.write("Transition curve resolved to UIViewAnimationCurve.EaseIn.", trace_1.categories.Transition);
                }
                return 1;
            case "easeOut":
                if (trace_1.isEnabled()) {
                    trace_1.write("Transition curve resolved to UIViewAnimationCurve.EaseOut.", trace_1.categories.Transition);
                }
                return 2;
            case "easeInOut":
                if (trace_1.isEnabled()) {
                    trace_1.write("Transition curve resolved to UIViewAnimationCurve.EaseInOut.", trace_1.categories.Transition);
                }
                return 0;
            case "linear":
                if (trace_1.isEnabled()) {
                    trace_1.write("Transition curve resolved to UIViewAnimationCurve.Linear.", trace_1.categories.Transition);
                }
                return 3;
            default:
                if (trace_1.isEnabled()) {
                    trace_1.write("Transition curve resolved to original: " + transition.curve, trace_1.categories.Transition);
                }
