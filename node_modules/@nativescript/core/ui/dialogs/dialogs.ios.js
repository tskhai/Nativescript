    ...
    return new Promise(function (resolve, reject) {
        try {
            var options = !dialogs_common_1.isDialogOptions(arg)
                ? { title: dialogs_common_1.ALERT, okButtonText: dialogs_common_1.OK, message: arg + "" }
                : arg;
            ...
            if (options) {
                switch (options.capitalizationType) {
                    case dialogs_common_1.capitalizationType.all: {
                        textField_1.autocapitalizationType =
                            3;
                        break;
                    }
            ...
            if (options.actions) {
                for (i = 0; i < options.actions.length; i++) {
                    ...
                    if (types_1.isString(action_1)) {
                        var thisActionIsDestructive = options.destructiveActionsIndexes &&
                            options.destructiveActionsIndexes.indexOf(i) !== -1;
                        var dialogType = thisActionIsDestructive
                            ? 2
                            : 0;
                        alertController.addAction(UIAlertAction.actionWithTitleStyleHandler(action_1, dialogType, function (arg) {
                            resolve(arg.title);
                        }));
                    }
                }
