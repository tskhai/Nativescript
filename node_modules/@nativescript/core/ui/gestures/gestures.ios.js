    ...
    GesturesObserver.prototype._attach = function (target, type) {
        ...
        if (target && target.nativeViewProtected && target.nativeViewProtected.addGestureRecognizer) {
            ...
            if (type & gestures_common_1.GestureTypes.tap) {
                nativeView.addGestureRecognizer(this._createRecognizer(gestures_common_1.GestureTypes.tap, function (args) {
                    if (args.view) {
                        _this._executeCallback(_getTapData(args));
                    }
                }));
            }
            if (type & gestures_common_1.GestureTypes.doubleTap) {
                nativeView.addGestureRecognizer(this._createRecognizer(gestures_common_1.GestureTypes.doubleTap, function (args) {
                    if (args.view) {
                        _this._executeCallback(_getTapData(args));
                    }
                }));
...
function _getTapData(args) {
    ...
    return {
        ...
        object: args.object,
        getPointerCount: function () { return recognizer.numberOfTouches; },
        getX: function () { return utils_1.layout.toDeviceIndependentPixels(center.x); },
        getY: function () { return utils_1.layout.toDeviceIndependentPixels(center.y); }
    };
}
