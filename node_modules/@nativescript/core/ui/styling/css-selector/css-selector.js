...
var Selector = (function (_super) {
    ...
    function Selector(selectors) {
        ...
        for (var i = selectors.length - 1; i > -1; i--) {
            ...
            _this.specificity += sel.specificity;
            if (sel.dynamic) {
                _this.dynamic = true;
            }
        ...
        _this.last = selectors[selectors.length - 1];
        return _this;
    }
...
(function (Selector) {
    var ChildGroup = (function () {
        ...
        ChildGroup.prototype.match = function (node) {
            return this.selectors.every(function (sel, i) { return (node = (i === 0 ? node : node.parent)) && sel.match(node); }) ? node : null;
        };
        ChildGroup.prototype.mayMatch = function (node) {
            return this.selectors.every(function (sel, i) { return (node = (i === 0 ? node : node.parent)) && sel.mayMatch(node); }) ? node : null;
        };
        ChildGroup.prototype.trackChanges = function (node, map) {
            this.selectors.forEach(function (sel, i) { return (node = (i === 0 ? node : node.parent)) && sel.trackChanges(node, map); });
        };
        return ChildGroup;
    }());
    ...
    var SiblingGroup = (function () {
        ...
        SiblingGroup.prototype.match = function (node) {
            return this.selectors.every(function (sel, i) { return (node = (i === 0 ? node : getNodeDirectSibling(node))) && sel.match(node); }) ? node : null;
        };
        SiblingGroup.prototype.mayMatch = function (node) {
            return this.selectors.every(function (sel, i) { return (node = (i === 0 ? node : getNodeDirectSibling(node))) && sel.mayMatch(node); }) ? node : null;
        };
        SiblingGroup.prototype.trackChanges = function (node, map) {
            this.selectors.forEach(function (sel, i) { return (node = (i === 0 ? node : getNodeDirectSibling(node))) && sel.trackChanges(node, map); });
        };
        return SiblingGroup;
    }());
...
var SelectorsMap = (function () {
    ...
    SelectorsMap.prototype.query = function (node) {
        ...
        var cssClasses = node.cssClasses, id = node.id, cssType = node.cssType;
        var selectorClasses = [
            this.universal,
            this.id[id],
            this.type[cssType]
        ...
        if (cssClasses && cssClasses.size) {
            cssClasses.forEach(function (c) { return selectorClasses.push(_this.class[c]); });
        }
        var selectors = selectorClasses.reduce(function (cur, next) { return cur.concat(next || []); }, []);
        selectorsMatch.selectors = selectors
            .filter(function (sel) { return sel.accumulateChanges(node, selectorsMatch); })
            .sort(function (a, b) { return a.specificity - b.specificity || a.pos - b.pos; });
